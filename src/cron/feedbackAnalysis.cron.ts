...
export class FeedbackAnalysisCronService {
  ...
  /**
   * Analyze patterns in ignored comments
   */
  private async analyzeIgnorePatterns(ignoredComments: any[]) {
    const analysis = {
      issuePatterns: new Map<string, { count: number; reasons: string[] }>(),
      categoryPatterns: new Map<string, { count: number; reasons: string[] }>(),
      commonReasons: new Map<string, number>(),
    };

    for (const comment of ignoredComments) {
      // Track issue patterns
      const issueKey = comment.issue;
      if (!analysis.issuePatterns.has(issueKey)) {
        analysis.issuePatterns.set(issueKey, { count: 0, reasons: [] });
      }
      const issuePattern = analysis.issuePatterns.get(issueKey);
      issuePattern.count++;
      if (comment.ignoreReason) {
        issuePattern.reasons.push(comment.ignoreReason);
      }

      // Track category patterns
      const categoryKey = comment.issueCategory;
      if (!analysis.categoryPatterns.has(categoryKey)) {
        analysis.categoryPatterns.set(categoryKey, { count: 0, reasons: [] });
      }
      const categoryPattern = analysis.categoryPatterns.get(categoryKey);
      categoryPattern.count++;
      if (comment.ignoreReason) {
        categoryPattern.reasons.push(comment.ignoreReason);
      }

      // Track common reasons
      if (comment.ignoreReason) {
        const reason = comment.ignoreReason.toLowerCase();
        analysis.commonReasons.set(
          reason,
          (analysis.commonReasons.get(reason) || 0) + 1,
        );
      }
    }
  ...
  /**
   * Update repository settings based on feedback analysis
   */
  private async updateRepositorySettingsBasedOnFeedback(
    organizationId: string,
    analysis: any,
    ignoredComments: any[],
  ) {
  ...
  private async updateRepositorySettings(
    ...
    // Check if any issues are being ignored frequently
    for (const [issue, pattern] of analysis.issuePatterns) {
      if (pattern.count >= 3) {
        // If ignored 3+ times in a week
        const setting = repository.repositorySettings.find(
          (s) => s.key === issue && s.active,
        );

        if (setting) {
          // Get current prompt (use customPrompt if available, otherwise use default prompt)
          const currentPrompt =
            setting.customPrompt ||
            setting.prompt ||
            `Default prompt for ${issue}`;

          // Prepare feedback data for Gemini analysis
          const feedbackData = {
            reasons: pattern.reasons,
            count: pattern.count,
            examples: ignoredComments
              .filter((comment) => comment.issue === issue)
              .slice(0, 5),
          };

          // Create improved prompt using Gemini's smartest model
          const improvedPrompt = await this.generateImprovedPromptWithGemini(
            issue,
            currentPrompt,
            feedbackData,
          ...
          this.logger.log(
            `Updating prompt for issue "${issue}" in repository "${repository.name}" based on ${pattern.count} ignore feedbacks using Gemini AI`,
          );
        }
      }
    }
    ...
    for (const update of updates) {
      await this.prisma.repositorySettings.update({
        where: { id: update.id },
        data: { customPrompt: update.customPrompt },
      });
    }
  }
